...
$(window).on("load", function () {
    ...
    $("#update-config").on('click', function () {
        $.post(ctx + "config/getAll", function (r) {
            $breadcrumb.html("").append('<li class="breadcrumb-item">系统参数修改</li>');
            $main_content.html("").append(r);
        });
    });
    $("#get_view_log").on('click', function () {
        $.post(ctx + "view/excel", function (r) {
            if (r.code === 0) {
                window.location.href = "common/download?fileName=" + r.msg + "&delete=" + false;
            } else {
                $MB.n_warning(r.msg);
            }
        });
    });
    $("#get_view_log_add").on('click', function () {
        $.post(ctx + "view/excelAdd", function (r) {
            if (r.code === 0) {
                window.location.href = "common/download?fileName=" + r.msg + "&delete=" + false;
            } else {
                $MB.n_warning(r.msg);
    ...
    $("#get_view_log_add_7").on('click', function () {
        $.post(ctx + "view/excelAdd7", function (r) {
            if (r.code === 0) {
                window.location.href = "common/download?fileName=" + r.msg + "&delete=" + false;
            } else {
                $MB.n_warning(r.msg);
